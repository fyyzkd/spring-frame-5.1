...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	 ...
	 * 即spring默认扫描@Component、@Controller、@Service、@Repository注解的Bean
	 * 同时也支持Java EE 的@ManageredBean和JSR-300的@Name注解
	 */
	 ...
	 * 调用类路径Bean定义扫描器入口方法，返回的是新注册的Bean的个数
	 */
	public int scan(String... basePackages) {
		...
		// 返回注册的Bean的个数（当前注册的Bean个数-已经注册的Bean个数）
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			...
			for (BeanDefinition candidate : candidates) {
				...
				// 如果扫描到的Bean不是Spring的注解的Bean，则为Bean设置默认值
				// 设置Bean的自动依赖注入装配属性等
				if (candidate instanceof AbstractBeanDefinition) {
					postProcessBeanDefinition((AbstractBeanDefinition) candidate, beanName);
				}
				// 如果扫描到的Bean是Spring的注解Bean，则处理Bean定义中的通用注解
				if (candidate instanceof AnnotatedBeanDefinition) {
					// 处理注解Bean中通用的注解
					AnnotationConfigUtils.processCommonDefinitionAnnotations((AnnotatedBeanDefinition) candidate);
				}
				// 根据Bean名称检查指定的Bean是否需要在容器中注册，或者容器中是否存在冲突
