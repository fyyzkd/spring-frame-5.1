...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		if (earlySingletonExposure) {
			...
			if (earlySingletonReference != null) {
				// 根据名称获取的已注册的Bean和正在实例化的bean 是同一个
				if (exposedObject == bean) {
					// 当前实例化的Bean初始化完成
					exposedObject = earlySingletonReference;
				}
				// 当前Bean依赖其他Bean，并且发生循环引用时不允许创建新的实例对象
	 ...
	 * @see #autowireConstructor
	 * @see #instantiateBean
	 * 根据指定的策略模式，使用简单工厂，工厂方法或者容器的自动装配特性生成java实例对象
	 */
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		/**  获取容器在解析Bean定义资源时为BeanFactory设置的属性值 **/
		...
		if (mbd.getResolvedAutowireMode() == AUTOWIRE_BY_NAME || mbd.getResolvedAutowireMode() == AUTOWIRE_BY_TYPE) {
			...
			/**  根据Bean名称进行autowiring 自动装配处理 **/
			if (mbd.getResolvedAutowireMode() == AUTOWIRE_BY_NAME) {
				autowireByName(beanName, mbd, bw, newPvs);
			}
			// Add property values based on autowire by type if applicable.
			/**  根据bean 类型进行autowring自动装配处理 **/
	...
	protected void autowireByType(
		...
		/** 对Bean对象中非简单属性（不是简单继承的对象，如8种原始类型、字符、URL等都是简单处理）进行处理**/
		String[] propertyNames = unsatisfiedNonSimpleProperties(mbd, bw);
		for (String propertyName : propertyNames) {
			try {
				/** 获取指定属性名称的属性描述器 **/
				...
				/** 不对Object类型的属性进行autowring自动依赖注入**/
				if (Object.class != pd.getPropertyType()) {
					...
					/** 根据容器的Bean定义解析关系，返回所有要被注入的Bean对象 **/
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		for (PropertyValue pv : original) {
			...
			if (pv.isConverted()) {
				...
				/**
				 * 属性是可以转换的，且属性原始值是字符串类型，属性的原始类型值不是动态生成的字符串
				 * 属性的原始值不是集合或者数组类型的
				 **/
		...
		// 进行属性的依赖注入
		try {
			/**  最终调用的是AbstractNestablePropertyAccessor#setPropertyValue(PropertyTokenHolder, PropertyValue) **/
